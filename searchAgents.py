...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if nextPosition in self.corners and not visitedCorners[self.corners.index(nextPosition)]:
                visitedList = list(visitedCorners)
                visitedList[self.corners.index(nextPosition)] = True
                nextVisitedCorners = tuple(visitedList)
            else:
...
def cornersHeuristic(state, problem):
    ...
    for order in itertools.permutations(unvisited):
        ...
        for corner in order:
            total += abs(pos[0] - corner[0]) + abs(pos[1] - corner[1])
            pos = corner
